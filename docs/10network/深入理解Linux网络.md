...
+ 4）我的机器上出现了3万多个TIME_WAIT，内存开销会不会很大？

#### 7.2、Linux内核如何管理内存

+ 内核使用**SLAB/SLUB的内存管理机制**，总共分4步
...
+ 通过`dmidecode`命令查看主板上插着的CPU的详细信息
...
+ 用`cat /proc/pagetypeinfo`查看当前系统中伙伴系统各个尺寸的可用连续内存块数量

##### 7.2.4、slab分配器

+ 目前介绍的内存分配都是**以页面（4KB）为单位的**
...
+ TCP cache下每个slab占用8个页面，也就是8 乘 4096B = 32768B
...
+ **我们大概知道了一条ESTABLISH状态的空连接消耗的内存大约是3KB多一点点**
