...
### chap10、页面帧回收  149（164/674）
+ 第1个讨论
+ 第2个讨论，页面高速缓存
+ 第3个讨论，LRU链表
#### 10.1、页面替换策略
+ LRU，并不是完全准确。
+ Linux中的LRU链包含两个链，分别是`active_list`和`inactive_list`
#### 10.2、页面高速缓存
#### 10.7、2.6中有哪些新特性

### chap11、交换管理  165（180/674）
#### 11.1、描述交换区
#### 11.2、映射页表项到交换项
#### 1.3、分配一个交换槽
#### 11.4、
...
### chap12、共享内存虚拟文件系统  180（195/674）
+ 内核利用shm为匿名页创建后援文件并由shmget()生成后援区域。该文件系统由`kern_mount()`加载，所以它在内部加载，并不为用户所见
+ tmpfs是一个临时文件系统，它可以有选择地加载到/tmp/以获得一个基于RAM的临时文件系统。tmpfs的另一个用途是被加载到/dev/shm
#### 12.1、初始化虚拟文件系统
+ `init_tmpfs()`初始化虚拟文件系统，`kern_mount()`
+ linux/shmem_fs.h中
#### 12.2、使用shmem函数
+ `hmem_aops`和`shmem_vm_ops`分别具有`struct address_space_operations`和`struct vm_opertions_struct`，分别对应于**换页中的缺页处理和写页面到后援存储区**
#### 12.3、在tmpfs中创建文件
#### 12.4、虚拟文件中的缺页中断
##### 12.4.1、定位交换页面
##### 12.4.2、向交换区写页面
#### 12.5、tmps中的文件操作
+ 虚拟文件支持4种操作，**mmap()、read()、write()和fsync()**，这些函数的指针都存放在`shmem_file_operations`中
#### 12.6、tmps中索引节点操作
#### 12.7、建立共享区
#### 12.8、System V IPC
#### 12.9、2.6中有哪些新特性
